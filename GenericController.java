...
public class GenericController {
    ...
    public void initialize() {
        ...
        //Edit Student
        nameCol.setCellFactory(TextFieldTableCell.forTableColumn()); //Para conseguir editar a coluna nome to student
    ...
    public void handleFloorChoiceAction(ActionEvent actionEvent) {
        ...
        if (place != null) {
            ...
            if (floor.compareTo("All Floors") == 0) {
                ...
                createGraphGroupFloor(buildingMap.getAllNodesByfloor(Integer.parseInt(floor)));
            }
        }
    }
    ...
    public void handleAddStudentAction(ActionEvent actionEvent) {
        u = universityST.get("UFP");
        try {
            String a = studentRegistrationTextField.getText();
            String[] c = a.split(" ");
    ...
    private Node findClosestNode(double x, double y, int floor) {
        ...
        for (Node n : nodes) {
            if (n.getFloor() == floor) {
                doubles.add(Math.abs(Math.sqrt(Math.pow(n.getX() - x, 2) + Math.pow(n.getY() - y, 2))));
                nodesArr.add(n);
            }
        }
        int mark = 0;
